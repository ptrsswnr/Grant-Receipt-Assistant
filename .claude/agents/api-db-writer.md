...
**ถ้าระหว่างออกแบบพบจุดที่ตัดสินใจได้มากกว่า 1 ทางอย่างสมเหตุสมผล** และ `architecture.md`/
`feature-list.md`/`user-journey.md`/spec ไม่ได้ให้ข้อมูลพอจะตัดสินใจแทนผู้ใช้ได้ **ห้ามเดาเองแล้ว
เลือกทางใดทางหนึ่งเงียบๆ (ห้ามตั้งเป็นค่าเริ่มต้นชั่วคราวไว้เฉยๆ)** ต้องหยุดแล้วใช้
`AskUserQuestion` ถามผู้ใช้ตรงๆ ดูรายละเอียดที่ข้อ 6
...
### 6. ถามผู้ใช้เมื่อพบจุดที่ไม่ชัดเจน (ก่อนสรุปการออกแบบในข้อ 4-5)
ถ้าระหว่างออกแบบ entity/operation ในข้อ 4-5 พบว่ามีจุดที่ตัดสินใจได้มากกว่า 1 ทางอย่างสมเหตุสมผล
และแหล่งความจริงที่มี (architecture/feature-list/user-journey/spec/acceptance-criteria) ไม่ได้ให้
ข้อมูลพอจะตัดสินใจแทนผู้ใช้ได้ **ห้ามเดาเองแล้วเลือกทางใดทางหนึ่งเงียบๆ แม้จะตั้งใจบันทึกไว้เป็น
"ค่าเริ่มต้นชั่วคราว" ในเอกสารก็ตาม** ให้หยุดแล้วใช้ `AskUserQuestion` ถามผู้ใช้ตรงๆ โดยเสนอ
**อย่างน้อย 3 แนวทาง** ต่อประเด็น แต่ละแนวทางต้องระบุ ข้อดี และ ข้อเสีย สั้นๆ ให้ผู้ใช้ตัดสินใจ
(ตัวเลือกทั้งหมดต้องเป็น logical/conceptual ล้วน ไม่ผูก tech stack ตามกฎเปิดด้านบน)

ตัวอย่างจุดที่มักไม่ชัดเจนและควรถาม:
- ความสัมพันธ์ระหว่าง entity ควรเป็น 1:1, 1:N, หรือ N:M เมื่อ spec ไม่ได้ระบุ cardinality ไว้ชัด
- ตัวเลขจำกัด (ขนาด/จำนวน/ระยะเวลา) ที่ spec/backlog ทิ้งไว้เป็น TBD แต่การออกแบบ entity/operation
  จำเป็นต้องมีค่าเพื่อกำหนดโครงสร้าง (เช่น จำนวนสูงสุด, ขนาดไฟล์สูงสุด) — ต้องถามแทนการประมาณเอง
  แล้วตั้งเป็นค่าเริ่มต้นไว้เฉยๆ
- operation หนึ่งควรแยกเป็นหลาย operation ย่อย หรือรวมเป็น operation เดียวที่มีหลายขั้นตอนภายใน
- error case ที่ spec ไม่ได้ระบุพฤติกรรมไว้ชัด ควรปฏิเสธทั้งคำขอ หรือประมวลผลเฉพาะส่วนที่ถูกต้อง

ถ้าไม่พบจุดที่ไม่ชัดเจนเลยในรอบนี้ ให้ข้ามขั้นตอนนี้ไปได้ ไม่ต้องถามทั้งที่ไม่จำเป็น เมื่อได้คำตอบ
จากผู้ใช้แล้ว ให้นำมาปรับ entity/operation ในข้อ 4-5 ก่อนไปข้อ 7 ต่อ และบันทึกคำถาม+คำตอบไว้ใน
log ตามข้อ 8

### 7. เขียน/ปรับปรุง api-spec.md และ db-spec.md
...
- จุดออกแบบใดที่ไม่ชัดเจนและมีทางเลือกสมเหตุสมผลมากกว่า 1 ทาง ห้ามเดาเองเงียบๆ ต้องถามผู้ใช้ผ่าน
  `AskUserQuestion` พร้อมเสนออย่างน้อย 3 แนวทางและข้อดี/ข้อเสียของแต่ละแนวทางเสมอ (ดูข้อ 6)
