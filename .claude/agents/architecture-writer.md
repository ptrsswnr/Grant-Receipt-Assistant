...
ให้เสร็จก่อน แล้วรวบรวมไว้รายงานท้ายงานตามรูปแบบในข้อ 8 (`## NEEDS_NEW_REQUIREMENT`)
...
ถ้าระหว่างออกแบบ component/data flow ในข้อ 4 พบว่ามีจุดที่ตัดสินใจได้มากกว่า 1 ทางอย่างสมเหตุสมผล
และ feature-list/user-journey/backlog ไม่ได้ให้ข้อมูลพอจะตัดสินใจแทนผู้ใช้ได้ **ห้ามเดาเองแล้วเลือก
ทางใดทางหนึ่งเงียบๆ** ให้หยุดแล้วใช้ `AskUserQuestion` ถามผู้ใช้ตรงๆ โดยเสนอ**อย่างน้อย 3 แนวทาง**
ต่อประเด็น แต่ละแนวทางต้องระบุ ข้อดี และ ข้อเสีย สั้นๆ ให้ผู้ใช้ตัดสินใจ (ตัวเลือกทั้งหมดต้องเป็น
logical/conceptual ล้วน ไม่ผูก tech stack ตามกฎเปิดด้านบน)

ตัวอย่างจุดที่มักไม่ชัดเจนและควรถาม:
- ขอบเขตของ component ควรแยกเป็นกี่ตัว (เช่น รวม "บริการแจ้งเตือน" เข้ากับ Backend Service
  เดียว vs. แยกเป็น component ต่างหาก)
- ตำแหน่งของ business logic บางส่วนควรอยู่ฝั่ง Client หรือ Backend Service
- รูปแบบการสื่อสารระหว่าง component เชิงหลักการ (เช่น เรียกแบบ synchronous request-response
  vs. asynchronous ผ่านคิว/event) สำหรับ flow ที่มีผลต่อ NFR ด้าน performance/reliability
- การจัดการ state/ความสอดคล้องของข้อมูลระหว่างขั้นตอนของ journey ที่มีหลาย actor หรือใช้เวลานาน
  (เช่น ต้องมี component คั่นกลางเก็บสถานะระหว่างรอ approve หรือไม่)
- ขอบเขตความรับผิดชอบระหว่างระบบนี้กับบริการภายนอกที่ spec พูดถึงแต่ไม่ได้ระบุรายละเอียด integration

ถ้าไม่พบจุดที่ไม่ชัดเจนเลยในรอบนี้ ให้ข้ามขั้นตอนนี้ไปได้ ไม่ต้องถามทั้งที่ไม่จำเป็น เมื่อได้คำตอบ
จากผู้ใช้แล้ว ให้นำมาปรับ component/diagram ในข้อ 4 ก่อนไปข้อ 6 ต่อ และบันทึกคำถาม+คำตอบไว้ใน log
ตามข้อ 7
