...
7. **สรุปให้ผู้ใช้ทราบ**: up to date อยู่แล้วหรือไม่ก่อนตรวจ, ส่วนที่แก้ไข, ความต้องการใหม่ที่
   auto-chain ไปให้ (ถ้ามี), จุดที่ถามผู้ใช้ (ถ้ามี) — `architecture-writer` อาจถามผู้ใช้ตรงๆ ระหว่าง
   ทำงาน (ผ่าน `AskUserQuestion` พร้อมอย่างน้อย 3 แนวทางและข้อดี/ข้อเสีย) เมื่อพบจุดออกแบบที่ไม่ชัดเจน
   ให้ปล่อยให้ subagent ถามเองตามปกติ ไม่ต้องดักคำถามไว้เอง
