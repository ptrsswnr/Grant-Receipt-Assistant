...
> `fundId`/`receiptId` เป็น input แทน
...
กฎ 2 ข้อที่บังคับกับทุก operation ใน [[api-spec]] (ตาม
...
2. **Admin ต้องถูกปฏิเสธจากทุก operation ในหมวดที่ 1–7, 10–12** (ใบเสร็จ/ทุน/consent/สิทธิ์ข้อมูล
   ส่วนบุคคล) ที่ระดับตรรกะสิทธิ์ ไม่ใช่แค่ซ่อนปุ่มใน UI — สิทธิ์ของ Admin จำกัดเฉพาะ
   [[admin-rule-management]] (OP-11, OP-12) เท่านั้น
...
```mermaid
sequenceDiagram
    actor U as ผู้เรียก (นักวิจัย หรือ Admin)
    participant C as Client
    participant B as Backend Service
    participant DS as Primary Data Store

    U->>C: เรียก operation ใดๆ ที่มี fundId/receiptId เป็น input
    C->>B: ส่งคำขอ (operation + fundId/receiptId)
    B->>DS: อ่าน Fund.ownerUserId (หรือผ่าน Receipt.fundId ถ้า input เป็น receiptId)
    DS-->>B: ownerUserId ของทุน/ใบเสร็จนั้น
    alt operation อยู่ในหมวด 1-7, 10-12 และผู้เรียกมีบทบาท Admin
        B-->>C: ปฏิเสธ — Admin ไม่มีสิทธิ์เข้าถึงข้อมูลใบเสร็จส่วนบุคคลเลยในระดับตรรกะสิทธิ์ (ไม่ใช่แค่ซ่อน UI)
...
| Operation | มี `fundId`/`receiptId` เป็น input? | ตรวจสอบเจ้าของ? | Admin เรียกได้? |
|---|---|---|---|
| OP-01 อัปโหลดใบเสร็จ | ใช่ (`fundId`) | ใช่ | ไม่ได้ |
...
| OP-08 Dashboard | ใช่ (`fundId`) | ใช่ | ไม่ได้ |
| OP-09 รายการแจ้งเตือน | ไม่ (implicit ผู้เรียกปัจจุบัน) | ใช่ (กรองเฉพาะทุนของผู้เรียก) | ไม่ได้ |
| OP-10 Export รายงาน | ใช่ (`fundId`) | ใช่ | ไม่ได้ |
| OP-11 นำเข้าระเบียบ | ไม่ | ไม่เกี่ยว (ไม่มีข้อมูลใบเสร็จส่วนบุคคล) | **ได้ (เฉพาะ Admin)** |
| OP-12 ดูเวอร์ชัน active | ไม่ | ไม่เกี่ยว | **ได้ (เฉพาะ Admin)** |
...
|---|---|---|---|
| 1 | นักวิจัย A ทราบ/เดา ID ใบเสร็จหรือทุนของนักวิจัย B | ปฏิเสธการเข้าถึง ไม่แสดงข้อมูลของ B ให้ A เห็น — ไม่เปิดเผยด้วยว่า ID นั้นมีอยู่จริงหรือไม่ (ป้องกันการเดา ID แบบ enumeration) | FR-12 AC-2, NFR-05 |
| 2 | Admin พยายามเข้าถึงข้อมูลใบเสร็จส่วนบุคคลของนักวิจัยคนใดคนหนึ่ง | ปฏิเสธที่ระดับตรรกะสิทธิ์ ไม่ใช่แค่ซ่อนปุ่มใน UI | FR-12 AC-3 |
| 3 | นักวิจัยมีทุนวิจัยมากกว่า 1 ทุน | ใบเสร็จที่อัปโหลดผูกกับทุนที่เลือกเท่านั้น ไม่ปรากฏในทุนอื่นของนักวิจัยคนเดียวกัน | FR-01 AC-3 |
...
- [[db-spec#E-01 ผู้ใช้ระบบ (User)|db-spec E-01]], [[db-spec#E-02 ทุนวิจัย (Fund)|E-02]]
- [[architecture#2.1 ขอบเขตความรับผิดชอบต่อ Component|architecture 2.1]]
- ฟีเจอร์อื่นทั้งหมด (1-8, 10-12) — ทุกฟีเจอร์อ้างอิงกฎในเอกสารนี้เป็นกฎ cross-cutting
