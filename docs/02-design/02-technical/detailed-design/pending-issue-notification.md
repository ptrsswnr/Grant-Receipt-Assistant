...
> แปลงจาก [[user-journey#Journey 2: นักวิจัยติดตามภาพรวมโครงการและจัดการใบเสร็จที่มีปัญหาผ่าน Dashboard และการแจ้งเตือน|Journey 2 ขั้นตอนที่ 2]]
...
- **อัปเดต 2026-09-03**: แก้ข้อความ "ของทุนทั้งหมดที่ผู้เรียกเป็นเจ้าของ" เป็น "ของโครงการทั้งหมดที่
  ผู้เรียกเป็นเจ้าของ" และปรับรหัส entity ให้ตรงกับ [[db-spec]] ที่จัดเรียงใหม่ (Receipt = E-04,
  Project = E-03) — เปลี่ยนคำเรียกบทบาทเป็น "นักวิจัย/เจ้าของโครงการ"
...
ของโครงการวิจัยทั้งหมดที่ผู้เรียกเป็นเจ้าของ ไม่ใช่ entity ที่ persist ไว้ต่างหาก — ตรงกับที่
[[architecture#3.2 Journey 2 — ติดตามภาพรวมโครงการและจัดการใบเสร็จที่มีปัญหา (Dashboard + แจ้งเตือน)|architecture Journey 2]]
ออกแบบไว้
...
sequenceDiagram
    ...
    B->>DS: ค้นหา Receipt ที่ status ∈ {"ต้องแก้ไข","ไม่เข้าเงื่อนไข"} ของโครงการทั้งหมดที่ผู้เรียกเป็นเจ้าของ
...
|---|---|---|---|
| [[api-spec#OP-09 ดูรายการแจ้งเตือนใบเสร็จที่มีปัญหาค้างอยู่\|OP-09]] | [[db-spec#E-04 ใบเสร็จ (Receipt)\|Receipt]] (E-04) | อ่าน (`status`, `projectId`) | กรองเฉพาะ `status` ∈ {"ต้องแก้ไข","ไม่เข้าเงื่อนไข"} ของทุกโครงการที่ผู้เรียกเป็นเจ้าของ — ไม่มีการแก้ไข entity ใดในฟีเจอร์นี้ |
| OP-09 | [[db-spec#E-03 โครงการวิจัย (Project)\|Project]] (E-03) | อ่าน (`ownerUserId`) | ใช้กรองว่าเป็นโครงการของผู้เรียกเท่านั้น (implicit — ไม่ต้องรับ `projectId` เป็น input) |
...
| 3 | การแจ้งเตือนตามกำหนดเวลาส่งหลักฐานรายโครงการ | ไม่รวมอยู่ในขอบเขต — ตัดออกจาก MVP นี้แล้ว | [[20260816-01-grant-receipt-verification#2.2 ตัดออกจากขอบเขต MVP นี้ (ยืนยันโดยผู้ใช้เมื่อ 2026-08-16 — ดูหัวข้อ 7)]] |
