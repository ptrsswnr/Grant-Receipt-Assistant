...
> แปลงจาก [[user-journey#Journey 1: นักวิจัยอัปโหลดใบเสร็จและตรวจสอบผลจนผ่านเกณฑ์|Journey 1 ขั้นตอนที่ 8-9]]
...
ยืนยัน), FR-04 (Rule Engine ตัดสิน), FR-05 (LLM แปลผล), FR-06 (แสดงผล) เป็น operation เดียวเพราะ
pipeline เป็น synchronous request-response ตลอดสาย (ตัดสินใจแล้วที่
...
sequenceDiagram
    ...
    alt receiptId ไม่ใช่ของผู้เรียก
        ...
        DS-->>B: บันทึกสำเร็จ
        B->>DS: ดึง RuleVersion ที่ isActive = จริงปัจจุบัน (ผ่าน OP-12 ภายใน)
        DS-->>B: RuleVersion + RuleItem ทั้งหมดของเวอร์ชันนั้น
        alt ไม่มี RuleVersion active เลย (Admin ยังไม่เคยนำเข้าระเบียบ)
            B-->>C: ปฏิเสธ พร้อมสถานะ "ยังไม่มีระเบียบให้ใช้ตรวจ" (บล็อกจนกว่า Admin นำเข้าระเบียบแรก)
        else มี RuleVersion active
            B->>RE: (ลำดับ 2) ส่งค่าที่ยืนยันแล้ว + RuleVersion active ให้ตัดสิน
            RE-->>B: decisionStatus (ผ่าน/ต้องแก้ไข/ไม่เข้าเงื่อนไข) + RuleItem ที่อ้างอิง (อย่างน้อย 1 รายการเสมอ)
            B->>DS: (ลำดับ 3) สร้าง VerificationResult ใหม่ (ruleVersionId = snapshot เวอร์ชันที่ใช้ตัดสินจริง)
...
> ไม่ได้ระบุพฤติกรรมของกรณีนี้ไว้ตรงๆ ในหัวข้อ "กรณี Error หลัก" (ระบุแค่ 2 กรณี: เจ้าของไม่ตรง และ
> สถานะไม่ถูกต้อง) เอกสารนี้เลือกยึดตามลำดับการประมวลผลที่ระบุไว้ตรงตัวอยู่แล้ว (ลำดับ 1-5 ข้างบน)
> โดยตีความว่า **หากลำดับ 2 (Rule Engine) หรือ 4 (LLM) ล้มเหลวทางเทคนิค ระบบจะยังไม่สร้าง
> `VerificationResult`** เพราะ [[db-spec#4. กฎทางธุรกิจที่กระทบโครงสร้างข้อมูล (Business Rules)|db-spec กฎข้อ 3]]
> กำหนดให้ `VerificationResult` เป็น append-only (สร้างเมื่อมีผลตัดสินจริงเท่านั้น) และ
> [[db-spec#4. กฎทางธุรกิจที่กระทบโครงสร้างข้อมูล (Business Rules)|db-spec กฎ NFR-03]] กำหนดให้ทุกผล
> ตรวจต้องมีข้อระเบียบอ้างอิงอย่างน้อย 1 รายการเสมอ — ผลตรวจที่ยังไม่มีข้อระเบียบอ้างอิง (เช่น กรณี
> Rule Engine ล้มเหลวก่อนตอบ) จึงไม่ควรถูกสร้างขึ้นเลย ส่วน `Receipt.confirmed*` ที่บันทึกไปแล้วใน
> ลำดับ 1 **จะยังคงอยู่** (ไม่ rollback) และ `Receipt.status` ยังคงเป็น "รอตรวจทาน" เหมือนเดิม —
...
|---|---|---|---|
| [[api-spec#OP-04 ยืนยันข้อมูลใบเสร็จและส่งเข้าตรวจ\|OP-04]] | [[db-spec#E-03 ใบเสร็จ (Receipt)\|Receipt]] (E-03) | แก้ไข (`confirmed*`) | ลำดับ 1 — เกิดก่อนเรียก Rule Engine เสมอ (ไม่ทับ `ocr*`) |
| OP-04 | [[db-spec#E-04 เวอร์ชันระเบียบ (RuleVersion)\|RuleVersion]] (E-04) + [[db-spec#E-05 ข้อกำหนดย่อยระเบียบ (RuleItem)\|RuleItem]] (E-05) | อ่าน (`isActive` = จริง) | ลำดับ 2 — ต้องมี record active อย่างน้อย 1 ตัว ไม่เช่นนั้นบล็อกการตรวจ |
| OP-04 | [[db-spec#E-06 ผลตรวจใบเสร็จ (VerificationResult)\|VerificationResult]] (E-06) | สร้าง | ลำดับ 3 — `ruleVersionId` เป็น snapshot (ไม่ใช่ live reference), append-only |
| OP-04 | [[db-spec#E-07 ข้อระเบียบที่อ้างอิงในผลตรวจ (VerificationRuleCitation)\|VerificationRuleCitation]] (E-07) | สร้าง (≥1 record เสมอ) | ลำดับ 3 — ผูกกับ `VerificationResult` ที่สร้างในคำขอเดียวกัน |
| OP-04 | Receipt (E-03) | แก้ไข (`status` → ตาม `decisionStatus`) | ลำดับ 5 — หลัง LLM แปลผลสำเร็จแล้วเท่านั้น |
| [[api-spec#OP-05 ดูรายละเอียดผลตรวจใบเสร็จ\|OP-05]] | Receipt (E-03), VerificationResult (E-06), RuleItem (E-05) ผ่าน VerificationRuleCitation | อ่านทั้งหมด | read-only — ไม่มีการแก้ไข |
...
| 3 | ยังไม่มี RuleVersion ที่ active เลย (Admin ยังไม่นำเข้าระเบียบแรก) | คืนค่าว่างพร้อมสถานะ "ยังไม่มีระเบียบให้ใช้ตรวจ" บล็อกไม่ให้ตัดสินใจได้จนกว่า Admin จะนำเข้าระเบียบแรก | [[api-spec#OP-12 ดูเวอร์ชันระเบียบที่ active ปัจจุบัน\|OP-12]] |
...
- [[db-spec#E-06 ผลตรวจใบเสร็จ (VerificationResult)|db-spec E-06]], [[db-spec#E-07 ข้อระเบียบที่อ้างอิงในผลตรวจ (VerificationRuleCitation)|E-07]]
