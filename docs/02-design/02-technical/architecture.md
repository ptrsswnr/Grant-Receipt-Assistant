...
เจ้าหน้าที่การเงินของมหาวิทยาลัย (ซึ่งอยู่นอกระบบทั้งหมด) ระบบมี 2 บทบาท: **นักวิจัย/เจ้าของทุน** และ
**Admin** (ดู [[20260816-01-grant-receipt-verification#3. บทบาทผู้ใช้ (User Roles)]]) หลักการ
...
flowchart LR
    subgraph Actors["ผู้ใช้งาน"]
        Researcher["นักวิจัย/เจ้าของทุน"]
        Admin["Admin (ผู้ดูแลระบบ/ผู้อัปเดตระเบียบ)"]
    end

    Client["ฝั่งไคลเอนต์ / หน้าจอผู้ใช้ (Client)<br/>UI ทุกหน้าจอ — ส่งไฟล์ทั้งชุด (สูงสุด 5 ไฟล์/รายการ)<br/>ตรงไปยัง Backend Service + แสดงคำเตือน "อัปโหลดครั้งละ<br/>1 รายการเท่านั้น" ไม่มีการตรวจสอบไฟล์ฝั่ง Client"]

    Backend["บริการฝั่งเซิร์ฟเวอร์ (Backend Service)<br/>Access Control · Upload Validation (จุดเดียว —<br/>≤5 ไฟล์/รายการ, ≤5 MB/ไฟล์) · Orchestration ·<br/>Consent/Erasure · Dashboard/Export/Notification · Audit Logging"]

    RuleEngine["เอนจินตรวจสอบตามกฎระเบียบ<br/>(Verification Rule Engine)<br/>ผู้ตัดสินผ่าน/ต้องแก้ไข/ไม่เข้าเงื่อนไข"]
    ...
    DataStore["ที่เก็บข้อมูลหลัก (Primary Data Store)<br/>ผู้ใช้/ทุน/ใบเสร็จ (1 record : N ไฟล์ ≤5)/<br/>ผลตรวจ/เวอร์ชันระเบียบ/consent"]
...
|---|---|---|
| **ฝั่งไคลเอนต์ / Client** | เรนเดอร์ทุกหน้าจอ (upload, ตรวจทาน OCR, สถานะใบเสร็จ, Dashboard, consent/privacy, data access/erasure, admin rule import); รับไฟล์ทั้งชุดของใบเสร็จ 1 รายการจากผู้ใช้ (สูงสุด 5 ไฟล์) แล้วส่งตรงไปยัง Backend Service **โดยไม่ตรวจสอบไฟล์เอง** (ตัดสินใจแล้ว 2026-08-23 — ดูหัวข้อ 5.1: Backend Service เป็นผู้ตรวจสอบไฟล์ที่ authoritative เพียงจุดเดียว); แสดงข้อความเตือน/คำแนะนำระหว่างอัปโหลดว่าให้อัปโหลดครั้งละ 1 รายการ (1 ชุดเอกสาร) เท่านั้น เพื่อป้องกันความสับสนปนกันหลายรายการ (FR-22) | FR-01, FR-03, FR-08, FR-13, FR-14, FR-16, FR-17, FR-18, FR-22, NFR-02 |
| **Backend Service** | บังคับสิทธิ์การเข้าถึงตามเจ้าของทุกคำขอ; **เป็นจุดเดียวที่ตรวจสอบไฟล์อัปโหลดอย่างเป็นทางการทั้งชุด** ต่อใบเสร็จ 1 รายการ (ชนิดไฟล์/ไฟล์ไม่เสียหาย/ขนาดแต่ละไฟล์ไม่เกิน **5 MB** /จำนวนไฟล์ไม่เกิน **5 ไฟล์ต่อรายการ** — ตัวเลข finalize แล้วตาม FR-19(3)(4)/FR-22 ไม่ใช่ค่าประมาณอีกต่อไป กำหนดร่วมกันเพื่อไม่ให้น้ำหนักรวมของทั้งชุดเอกสารกระทบความเร็วของ OCR ตาม NFR-06 — ตัดสินใจแล้ว 2026-08-23 ดูหัวข้อ 5.1); ผูกไฟล์ทั้งชุดที่ผ่านการตรวจสอบเข้ากับ receipt record เดียวกัน (1 ใบเสร็จ : N ไฟล์ ไม่ใช่ 1:1 อีกต่อไป — FR-22); orchestrate ลำดับ OCR → review → Rule Engine → LLM แบบ synchronous request-response ตลอด pipeline (ตัดสินใจแล้ว 2026-08-23 ดูหัวข้อ 5.2); จัดการสถานะใบเสร็จและ resubmission; สร้าง Dashboard/แจ้งเตือน/export พร้อม gate เงื่อนไข; จัดการวงจร consent (ขอ/บันทึก/ถอน/ลบทันที); จัดการคำขอ data access & erasure; เขียน audit event | FR-01, FR-03, FR-04(orchestrate), FR-06–FR-22 (ยกเว้นการตัดสินใจจริงที่อยู่ใน Rule Engine), NFR-02, NFR-04, NFR-05, NFR-06, NFR-11 |
| **Verification Rule Engine** | เก็บ/ใช้ระเบียบทุนวิจัยที่ Admin นำเข้าแบบมีเวอร์ชัน; ประเมินใบเสร็จเทียบระเบียบเวอร์ชันที่ active แล้วคืนคำตัดสิน (ผ่าน/ต้องแก้ไข/ไม่เข้าเงื่อนไข) พร้อมข้อระเบียบที่อ้างอิง — **เป็นผู้ตัดสินใจเพียงผู้เดียว ไม่ใช่ LLM** | FR-04, FR-06, FR-11, NFR-03, NFR-04 |
...
| **Primary Data Store** | เก็บบัญชีผู้ใช้/ทุนที่ถือ, **ใบเสร็จ 1 record ต่อ 1 ค่าใช้จ่าย ผูกได้กับหลายไฟล์ (1:N, สูงสุด 5 ไฟล์ต่อ record — ไม่ใช่ 1 ไฟล์ = 1 ใบเสร็จ อีกต่อไปตั้งแต่ FR-22)** + ข้อมูล OCR ของ record นั้น (draft/แก้ไขแล้ว รวมข้อมูลจากทุกไฟล์ในชุดเป็นผลลัพธ์เดียวต่อ record), ผลตรวจต่อใบเสร็จ (ต่อ record ไม่ใช่ต่อไฟล์), เวอร์ชันระเบียบ, หลักฐาน consent (วันที่/เวอร์ชัน Privacy Notice), สถานะ "ถูก export แล้วหรือยัง" ต่อใบเสร็จ (ต่อ record — ถ้า record ถูก export แล้ว ล็อกทั้งชุดไฟล์ ไม่ใช่ล็อกทีละไฟล์) | FR-01–FR-04, FR-07, FR-08, FR-10–FR-22, NFR-05, NFR-11 |
| **Audit Trail Store** | บันทึกที่ไม่ลบ/แก้ทับของ: เวอร์ชันระเบียบที่ใช้ตัดสินต่อใบเสร็จ, เหตุการณ์ consent (ให้/ถอน/ลบ), การนำเข้าระเบียบของ Admin — รองรับ dispute และ compliance | NFR-03, NFR-04, NFR-10 |
...
ข้อมูลที่ดึง (Journey 3 ดึงเฉพาะใบเสร็จสถานะผ่านของทุนหนึ่งก้อน ส่วน Journey 6 ดึงข้อมูลส่วนบุคคล
ทั้งหมดของนักวิจัยคนนั้น ได้แก่ ใบเสร็จ+ผลตรวจ+ประวัติ consent) จึงไม่วาดแยกซ้ำตามกฎข้อ 4 ของงานนี้

### 3.1 Journey 1 — อัปโหลดใบเสร็จและตรวจสอบผลจนผ่านเกณฑ์ (รวม consent gate + FR-19 + FR-22 multi-file bundle)
...
sequenceDiagram
    ...
    R->>C: เลือกไฟล์ทั้งชุดของค่าใช้จ่าย 1 เรื่อง (สูงสุด 5 ไฟล์/แผ่น) ผูกกับทุนของตนเอง (FR-01, FR-22)
    C->>B: ส่งไฟล์ทั้งชุด + metadata ทุน (Client ไม่ตรวจสอบไฟล์เอง — ส่งตรงไปยัง Backend Service)
    ...
    alt ไฟล์ไม่ถูกต้อง หรือจำนวนไฟล์เกิน 5 ไฟล์/รายการ
        ...
        B->>RE: ส่งข้อมูลใบเสร็จ + ระเบียบเวอร์ชัน active ปัจจุบัน
...
### 3.2 Journey 2 — ติดตามภาพรวมทุนและจัดการใบเสร็จที่มีปัญหา (Dashboard + แจ้งเตือน)
...
sequenceDiagram
    ...
    C->>B: ขอสรุปภาพรวมใบเสร็จของทุน
    B->>DS: อ่านสถานะใบเสร็จทั้งหมดของทุน (เฉพาะของเจ้าของ — FR-12)
    ...
    alt มีใบเสร็จต้องแก้ไข
        ...
        C-->>R: ใบเสร็จทั้งหมดของทุนผ่านตรวจแล้ว พร้อม export
    end
```
...
sequenceDiagram
    ...
    R->>C: เข้าสู่ระบบ เลือกทุนที่ต้องการยื่น (FR-12)
    C->>B: ขอสถานะใบเสร็จของทุน (ผ่าน Dashboard, FR-08)
    B->>DS: นับจำนวนใบเสร็จสถานะ "ผ่าน" ของทุนนี้
    ...
    alt ไม่มีใบเสร็จผ่านเลย
        ...
        B->>DS: รวบรวมใบเสร็จสถานะผ่านของทุน + สร้างไฟล์รายงาน
...
### 3.4 Journey 4 — Admin นำเข้า/อัปเดตระเบียบทุนวิจัยเข้าสู่ Rule Engine
...
sequenceDiagram
    ...
    A->>C: นำเข้า/อัปเดตระเบียบทุนวิจัยฉบับใหม่ (FR-11)
    C->>B: ส่งกฎ/เงื่อนไขที่อัปเดต
    B->>RE: บันทึกกฎเป็นเวอร์ชันใหม่ ไม่กระทบ LLM (FR-11)
    RE-->>B: ยืนยันเวอร์ชันใหม่ใช้งานได้ (active)
    B->>AT: บันทึกเวอร์ชันระเบียบที่นำเข้าไว้ (NFR-04)
    B-->>C: แจ้งผลสำเร็จ
    Note over RE: ใบเสร็จที่ส่งเข้าตรวจจากนี้ไปใช้ระเบียบเวอร์ชันใหม่ (เชื่อม Journey 1 ขั้นตอน Rule Engine)
```
...
| NFR-04 | Audit Trail — บันทึกกฎ/เวอร์ชันระเบียบ/ผลลัพธ์ต่อใบเสร็จ | Backend Service, Audit Trail Store, Verification Rule Engine | ทุกผลตรวจต้องผูกกับเวอร์ชันระเบียบที่ใช้ ณ ขณะนั้นแบบ immutable (แก้ไข/ลบทับไม่ได้) เพื่อรองรับการตรวจสอบย้อนหลังเมื่อระเบียบเปลี่ยน และรองรับ dispute |
| NFR-05 | Security/Access Control | Backend Service, Primary Data Store | Backend Service ต้องตรวจสอบสิทธิ์ตามเจ้าของข้อมูลทุก request ก่อนเข้าถึงข้อมูลใบเสร็จ — Admin ต้องถูกกันไม่ให้เข้าถึง endpoint ที่คืนข้อมูลใบเสร็จส่วนบุคคลได้เลยในระดับตรรกะสิทธิ์ ไม่ใช่แค่ซ่อนใน UI |
...
| NFR-07 | Scalability ปริมาณใบเสร็จ (เป้าหมายตัวเลขยังไม่กำหนด) | Backend Service, Primary Data Store | ยังไม่มีตัวเลขเป้าหมาย — รูปแบบการสื่อสารระหว่าง component (หัวข้อ 5.2) มีผลต่อว่าระบบต้องรองรับการประมวลผลพร้อมกันมากน้อยเพียงใด |
...
- [[backlog]] — สรุป FR/NFR ทั้งหมดที่ขับเคลื่อนการออกแบบนี้ (FR-01–FR-22, NFR-01–NFR-11)
- [[feature-list]] — 12 ฟีเจอร์ที่ map เข้ากับ component ในหัวข้อ 2.1
- [[user-journey]] — 7 journey ต้นทางของ Data Flow Diagram ในหัวข้อ 3
...
- [[technology-stack]] — ยังไม่มีเนื้อหา (ตรวจสอบแล้วเมื่อ 2026-08-23) — จุดที่จะกำหนด tech stack จริงในอนาคต
