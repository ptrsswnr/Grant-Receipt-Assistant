...
> [[technology-stack]] ยังไม่มีเนื้อหา (ตรวจสอบแล้ว ณ 2026-08-23) ชนิดข้อมูลทั้งหมดในเอกสารนี้ใช้คำ
...
- **อัปเดต 2026-08-23 (รอบ 2):** เพิ่ม **FR-22 (Multi-file Receipt Bundle)** เข้าสู่เอกสารนี้ ตามที่
  ...
  ปรับ entity `Receipt` (E-03) จาก "1 record : 1 ไฟล์" เป็น **"1 record : N ไฟล์ (1 ≤ N ≤ 5)"** โดย
  แยกข้อมูลระดับไฟล์ออกเป็น entity ใหม่ **E-13 ไฟล์ประกอบใบเสร็จ (ReceiptFile)** ตามหลัก
...
erDiagram
    USER {
        ...
        ข้อความ fullName
        ข้อความ email
        ข้อความ roleType "นักวิจัยเจ้าของทุน หรือ Admin"
...
บัญชีผู้ใช้แบบรวม ครอบคลุมทั้ง 2 บทบาท (นักวิจัย/เจ้าของทุน และ Admin) เพราะทั้งสองบทบาทต้องมีการ
เข้าสู่ระบบ/ยืนยันตัวตนเหมือนกัน ต่างกันที่สิทธิ์การเข้าถึงข้อมูล (ดู
...
| roleType | ข้อความ (ค่าที่เป็นไปได้: "นักวิจัย/เจ้าของทุน", "Admin") | ใช่ | กำหนดสิทธิ์การเข้าถึงทั้งหมด (FR-12, NFR-05) |
...
ทุนวิจัยที่นักวิจัยถือ เป็นเจ้าของ ใบเสร็จทุกใบต้องผูกกับทุนหนึ่งเสมอ (ดู
[[user-journey#Journey 1: นักวิจัยอัปโหลดใบเสร็จและตรวจสอบผลจนผ่านเกณฑ์|Journey 1 ขั้นตอนที่ 4]])

| Attribute | ชนิดข้อมูลเชิงตรรกะ | จำเป็นต้องมีค่า | คำอธิบาย |
|---|---|---|---|
| id | ตัวระบุ | ใช่ | รหัสอ้างอิงเฉพาะของทุน |
| fundName | ข้อความ | ใช่ | ชื่อทุนวิจัย |
| fundCode | ข้อความ | ไม่ | รหัสทุน/สัญญาทุน (ถ้ามี) |
| ownerUserId | อ้างอิงถึง Entity อื่น (User) | ใช่ | เจ้าของทุน ต้องเป็น User ที่ roleType = "นักวิจัย/เจ้าของทุน" เท่านั้น |
| createdAt | วันที่-เวลา | ใช่ | วันที่สร้างรายการทุนในระบบ |
...
**สมมติฐานที่ต้องบันทึกไว้**: เมื่อนักวิจัยถอนความยินยอมทั้งหมด (FR-20) ตัว entity ทุนวิจัย
**ไม่ถูกลบ** — FR-20 ระบุเฉพาะ "ข้อมูลใบเสร็จ/ผลตรวจ" เท่านั้น ทุนวิจัยยังคงอยู่เผื่อนักวิจัยกลับมาให้
ความยินยอมใหม่ในอนาคตแล้วอัปโหลดใบเสร็จผูกกับทุนเดิมได้ต่อ (ไม่ต้องสร้างทุนใหม่) — ถ้าผู้ใช้เห็นว่า
ควรลบทุนไปด้วย โปรดแจ้งกลับมาเพื่อปรับ

### E-03 ใบเสร็จ (Receipt)

หัวใจของระบบ — เป็น **record ระดับ "ค่าใช้จ่าย 1 เรื่อง"** ไม่ใช่ record ระดับไฟล์ (ตั้งแต่ FR-22:
1 record ผูกได้กับไฟล์ประกอบหลายไฟล์ผ่าน [[db-spec#E-13 ไฟล์ประกอบใบเสร็จ (ReceiptFile)|E-13]] —
...
มีแค่ลายเซ็นยืนยันตัวตน OCR จะดึงยอดเงินจากไฟล์ที่อ่านได้มาใส่ในค่าเดียวของ record นี้)

| Attribute | ชนิดข้อมูลเชิงตรรกะ | จำเป็นต้องมีค่า | คำอธิบาย |
|---|---|---|---|
| id | ตัวระบุ | ใช่ | รหัสอ้างอิงเฉพาะของใบเสร็จ (record ระดับค่าใช้จ่าย 1 เรื่อง) |
| fundId | อ้างอิงถึง Entity อื่น (Fund) | ใช่ | ทุนที่ใบเสร็จนี้ผูกอยู่ (FR-01) |
| uploadedAt | วันที่-เวลา | ใช่ | วันที่-เวลาที่สร้าง record นี้ครั้งแรก (คือครั้งที่อัปโหลดไฟล์ทั้งชุดครั้งแรก — ไม่ใช่ต่อไฟล์ แต่ละไฟล์มี `uploadedAt` ของตัวเองใน E-13) |
...
| status | ข้อความ (ค่าที่เป็นไปได้: "รอ OCR", "รอตรวจทาน", "ผ่าน", "ต้องแก้ไข", "ไม่เข้าเงื่อนไข") | ใช่ | สถานะปัจจุบันของใบเสร็จ (FR-06) — **ไม่มีค่า "รอผลตรวจ"** เพราะ pipeline การตรวจ (OP-04) เป็น synchronous ทั้งเส้นตาม [[architecture#5.2 รูปแบบการสื่อสารระหว่าง Backend Service ↔ OCR/Rule Engine/LLM — Synchronous หรือ Asynchronous|architecture 5.2]] สถานะจึงเปลี่ยนจาก "รอตรวจทาน" ตรงไปเป็นผลตัดสินสุดท้าย (ผ่าน/ต้องแก้ไข/ไม่เข้าเงื่อนไข) ในคำขอเดียวเสมอ ไม่มีช่วงเวลาที่ persist เป็น "รอผลตรวจ" จริง (แก้ไข 2026-08-23 — เดิมมีค่านี้อยู่แต่ไม่มี operation ใดตั้งค่านี้จริง ดู [[db-spec#5.4 สถานะ Receipt.status ที่ตัดออก (แก้ไข 2026-08-23)|หัวข้อ 5.4]]) |
...
### E-04 เวอร์ชันระเบียบ (RuleVersion)

ระเบียบทุนวิจัยของมหาวิทยาลัยที่ Admin นำเข้า มีเวอร์ชัน — มีได้เพียง **1 เวอร์ชันที่ active ในเวลา
เดียวกันทั้งระบบเท่านั้น** (ไม่ผูกกับทุนใดทุนหนึ่งเป็นการเฉพาะ เพราะ spec ต้นทางพูดถึง "ระเบียบทุนวิจัย
ของมหาวิทยาลัย" เป็นชุดเดียว ไม่ใช่รายทุน — ดู
[[architecture#3.1 Journey 1 — อัปโหลดใบเสร็จและตรวจสอบผลจนผ่านเกณฑ์ (รวม consent gate + FR-19)|Journey 1: "ระเบียบเวอร์ชัน active ปัจจุบัน"]])
...
| isActive | จริง/เท็จ | ใช่ | เป็นเวอร์ชันที่ใช้ตรวจใบเสร็จอยู่ในปัจจุบันหรือไม่ — มีได้สูงสุด 1 record ที่เป็นจริงทั้งระบบ |
...
### E-05 ข้อกำหนดย่อยระเบียบ (RuleItem)
...
### E-06 ผลตรวจใบเสร็จ (VerificationResult)
...
| ruleVersionId | อ้างอิงถึง Entity อื่น (RuleVersion) | ใช่ | เวอร์ชันระเบียบที่ **active ณ ขณะตรวจ** — บันทึกแบบ snapshot ไม่ใช่ reference ไปยัง "เวอร์ชันล่าสุดปัจจุบัน" เพื่อให้ผลตรวจเก่ายังอธิบายได้ถูกต้องแม้ระเบียบเปลี่ยนไปแล้ว (NFR-04) |
...
### E-07 ข้อระเบียบที่อ้างอิงในผลตรวจ (VerificationRuleCitation)
...
### E-08 เวอร์ชันประกาศความเป็นส่วนตัว (PrivacyNoticeVersion)
...
### E-09 ประวัติความยินยอม (ConsentRecord)
...
### E-10 รอบการ Export (ExportBatch)

หนึ่ง record ต่อการกด Export หนึ่งครั้ง — เก็บไว้เพื่อรองรับเหตุผล Audit Trail/ป้องกันการลบหลักฐาน
หนีความรับผิดที่ FR-18 อ้างถึง

| Attribute | ชนิดข้อมูลเชิงตรรกะ | จำเป็นต้องมีค่า | คำอธิบาย |
|---|---|---|---|
| id | ตัวระบุ | ใช่ | รหัสอ้างอิงเฉพาะ |
| fundId | อ้างอิงถึง Entity อื่น (Fund) | ใช่ | ทุนที่ถูก export รายงานสรุปในครั้งนี้ |
| exportedByUserId | อ้างอิงถึง Entity อื่น (User) | ใช่ | นักวิจัยผู้กด Export (ต้องเป็นเจ้าของทุนนั้น — FR-12) |
...
### E-11 ใบเสร็จในรอบ Export (ExportBatchReceipt)

Join entity ระหว่างรอบ export หนึ่งครั้งกับใบเสร็จที่ถูกรวมอยู่ในรอบนั้น — ใบเสร็จหนึ่งใบอาจถูกรวมอยู่
ใน export batch ได้มากกว่า 1 ครั้ง (เพราะทุกครั้งที่ export ระบบรวบรวม "ใบเสร็จสถานะผ่านทั้งหมดของทุน"
ใหม่เสมอ ไม่ใช่แค่ใบที่ผ่านใหม่ — ดู
...
### E-12 บันทึกเหตุการณ์ตรวจสอบย้อนหลัง (AuditLogEntry)
...
ขัดกับเจตนาการลบข้อมูลจริงตาม FR-18/FR-20
...
| affectedFundId | อ้างอิงถึง Entity อื่น (Fund) | ไม่ | ทุนที่เกี่ยวข้อง (ถ้ามี) |
...
### E-13 ไฟล์ประกอบใบเสร็จ (ReceiptFile)

**เพิ่มใหม่ 2026-08-23 (รอบ FR-22)** — แยกออกจาก `Receipt` (E-03) ตามหลัก normalization เพื่อรองรับ
...
|---|---|---|---|
| User | Fund | 1 : N | ผู้ใช้ 1 คน (บทบาทนักวิจัย) ถือทุนได้หลายทุน |
| User | ConsentRecord | 1 : N | ผู้ใช้ 1 คนมีประวัติให้/ถอนความยินยอมได้หลายครั้งตามเวลา |
| User | RuleVersion | 1 : N | Admin 1 คนนำเข้าระเบียบได้หลายเวอร์ชัน |
...
| Fund | Receipt | 1 : N | ทุน 1 ก้อนมีใบเสร็จได้หลายใบ |
| Fund | ExportBatch | 1 : N | ทุน 1 ก้อนถูก export ได้หลายครั้ง (คนละเวลา) |
...
2. **VerificationResult.ruleVersionId เป็น snapshot ไม่ใช่ live reference** — ต้องชี้ไปยัง
   RuleVersion ที่ `isActive = จริง` ณ ขณะตัดสินเท่านั้น แม้ Admin จะนำเข้าเวอร์ชันใหม่ในอนาคต
   ผลตรวจเก่าต้องยังอ้างอิงเวอร์ชันเดิมที่ใช้จริงตอนนั้นได้เสมอ (NFR-04)
...
5. **การลบใบเสร็จรายเดียว (FR-18)**: อนุญาตเมื่อ `Receipt.isExported = เท็จ` เท่านั้น ถ้าเป็นจริง
   ...
   ในชุด (E-13), และ `VerificationResult`/`VerificationRuleCitation` ที่ผูกอยู่ทั้งหมด แล้วสร้าง
   `AuditLogEntry` (eventType = "ลบใบเสร็จรายเดียว") ไว้เป็นหลักฐาน meta
6. **การถอนความยินยอมทั้งหมด (FR-20)**: ลบ `Receipt` ทุกใบของนักวิจัยคนนั้น **โดยไม่ตรวจสอบ
   `isExported`** (ต่างจากกฎข้อ 5) พร้อม `ReceiptFile`/`VerificationResult`/
   `VerificationRuleCitation`/`ExportBatchReceipt` ที่ผูกอยู่ — **ไม่ลบ** `Fund` (ดูสมมติฐานใน E-02)
   และไม่ลบ `ExportBatch` ตัวมันเอง (เก็บ record เปล่าไว้เป็นหลักฐาน meta ว่ามีการ export เกิดขึ้นจริง
   ในอดีต แม้ใบเสร็จที่อยู่ใน batch นั้นจะถูกลบไปแล้ว) แล้วสร้าง `AuditLogEntry` (eventType =
   "ถอนความยินยอม+ลบข้อมูลทั้งหมด") พร้อม `affectedReceiptCount`
7. **RuleVersion.isActive มีได้เพียง 1 record ที่เป็นจริงในเวลาเดียวกันทั้งระบบ** — การนำเข้าเวอร์ชัน
   ใหม่ (FR-11) ต้องตั้งเวอร์ชันเดิมเป็นเท็จในการดำเนินการเดียวกัน (ไม่ leave ทั้งสองเป็นจริงพร้อมกัน)
...
**ผลกระทบ**: แก้ enum ใน E-03 (หัวข้อ 2) เหลือ 5 ค่า ("รอ OCR", "รอตรวจทาน", "ผ่าน", "ต้องแก้ไข",
"ไม่เข้าเงื่อนไข") และแก้ [[api-spec#OP-08 ดูภาพรวมสถานะใบเสร็จของทุน (Dashboard)|api-spec OP-08]]
...
- [[architecture]] — component/data flow ต้นทางของการออกแบบนี้
- [[backlog]] — สรุป FR/NFR ทั้งหมด (FR-01–FR-22, NFR-01–NFR-11)
- [[feature-list]] — 12 ฟีเจอร์ที่ entity ในเอกสารนี้ต้องรองรับให้ครบ
